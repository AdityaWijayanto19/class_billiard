    ...
    <div class="min-h-screen bg-white dark:bg-[#050505] p-6 lg:p-10 transition-colors duration-300">
        ...
        <div
            ...
            <div class="space-y-1">
                ...
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white uppercase">Hero <span
                        style="color: var(--primary-color);">Configuration</span></h1>
                <p class="text-xs text-slate-500 dark:text-gray-500 font-medium">Atur identitas utama, logo brand, dan pesan
                    sambutan di halaman muka.</p>
            </div>
        </div>
            ...
            <div class="lg:col-span-5 space-y-8">
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">Brand
                            Logo Asset</label>
                        <x-tooltip>
                    ...
                    <div class="relative group aspect-video rounded-lg border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] overflow-hidden flex items-center justify-center p-8 transition-all duration-500"
                        ...
                        <div id="preview" class="w-full h-full flex items-center justify-center">
                            @if($hero && $hero->logo_image)
                                <img src="{{ asset('storage/' . $hero->logo_image) }}"
                            ...
                            @else
                                <div class="text-center space-y-3">
                                    ...
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Upload Brand Logo
                                    </p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">Background
                            Image</label>
                        <x-tooltip>
                            <p class="mb-2">Gunakan gambar rasio <strong>16:9</strong> dan ukuran maksimal
                                <strong>5MB</strong>.
                    ...
                    <div class="relative group aspect-video rounded-lg border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] overflow-hidden flex items-center justify-center p-8 transition-all duration-500"
                        @mouseenter="$el.style.borderColor = 'rgba(var(--primary-color-rgb), 0.3)'"
                        @mouseleave="$el.style.borderColor = ''">
                        <input type="file" name="background_image" id="backgroundInput"
                            class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" accept="image/*">
                        ...
                        <div id="backgroundPreview" class="w-full h-full flex items-center justify-center">
                            @if($hero && $hero->background_image)
                                <img src="{{ asset('storage/' . $hero->background_image) }}"
                                    class="max-w-full max-h-full object-cover group-hover:scale-105 transition-transform duration-700 rounded-lg">
                                <div
                                    class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all duration-300">
                                    <i class="ri-camera-switch-line text-3xl text-white"></i>
                                </div>
                            @else
                                <div class="text-center space-y-3">
                                    <i class="ri-image-add-line text-4xl text-slate-300 dark:text-white/10"></i>
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Upload Background
                                        Image</p>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="space-y-1">
                            ...
                            <p class="text-[9px] text-slate-400 uppercase tracking-tighter"
                                x-data="{ checked: {{ ($hero && $hero->is_active) ? 'true' : 'false' }} }"
                                x-text="checked ? 'Hero Section Active' : 'Hero Section Disabled'"></p>
                        </div>
            ...
            <div class="lg:col-span-7 space-y-8">
                <div
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-end">
                                <label
                                    class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">Main
                                    Display Title</label>
                                <span class="text-[9px] font-bold uppercase tracking-widest"
                                    style="color: var(--primary-color);">Large Text</span>
                            </div>
                            <input type="text" name="title" placeholder="e.g. CLASS" value="{{ $hero->title ?? 'CLASS' }}"
                                required
                                class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-md px-4 py-4 text-2xl font-bold tracking-tight text-slate-900 dark:text-white outline-none transition-all"
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-end">
                                <label
                                    class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">Banner
                                    Subtitle</label>
                                <span class="text-[9px] font-bold uppercase tracking-widest"
                                    style="color: var(--primary-color);">Red Banner Text</span>
                            </div>
                            <input type="text" name="subtitle" placeholder="e.g. BILLIARD"
                                value="{{ $hero->subtitle ?? 'BILLIARD' }}" required
                                class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-md px-4 py-3 text-sm font-bold text-slate-900 dark:text-white outline-none transition-all"
                                @focus="$el.style.borderColor = 'var(--primary-color)'" @blur="$el.style.borderColor = ''">
                        </div>
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">Tagline</label>
                            <input type="text" name="tagline" placeholder="e.g. Premium Billiard Lounge & Bar"
                                value="{{ $hero->tagline ?? 'Premium Billiard Lounge & Bar' }}"
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">CTA
                                Button 1 Text</label>
                            <input type="text" name="cta_text_1" placeholder="e.g. BOOK A TABLE"
                                value="{{ $hero->cta_text_1 ?? 'BOOK A TABLE' }}"
                                class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-md px-4 py-3 text-sm text-slate-900 dark:text-white outline-none transition-all"
                                @focus="$el.style.borderColor = 'var(--primary-color)'" @blur="$el.style.borderColor = ''">
                        </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center gap-2">
                                <label
                                    class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-gray-500">CTA
                                    Button 1 Link (Group WA / Booking)</label>
                                <x-tooltip>
                                     <p class="text-xs text-slate-400 mt-1">Masukkan link grup WhatsApp atau nomor (contoh:
                                        <code>081234567890</code>) untuk tombol "{{ $hero->cta_text_1 ?? 'BOOK A TABLE' }}". Sistem
                                        akan menormalisasi nomor menjadi <code>https://wa.me/&lt;number&gt;</code> saat disimpan.
                                    </p>
                                </x-tooltip>
                            </div>
                            <input type="text" name="cta_link_1"
                                placeholder="e.g. https://wa.me/6281234567890 or 081234567890"
                                value="{{ $hero->cta_link_1 ?? '' }}"
                                class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-md px-4 py-3 text-sm text-slate-900 dark:text-white outline-none transition-all"
                                @focus="$el.style.borderColor = 'var(--primary-color)'" @blur="$el.style.borderColor = ''">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="cta_text_2" value="{{ $hero->cta_text_2 ?? 'EXPLORE' }}"
                                class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-md px-4 py-3 text-sm text-slate-900 dark:text-white outline-none transition-all"
                                placeholder="e.g. EXPLORE" @focus="$el.style.borderColor = 'var(--primary-color)'"
                    ...
                    <div class="mt-12 pt-8 border-t border-slate-100 dark:border-white/5">
                        <button type="submit"
                            class="w-full text-black text-[10px] font-black uppercase tracking-widest py-4 rounded-md transition-all shadow-lg active:scale-95 flex items-center justify-center gap-3"
                            style="background-color: var(--primary-color); box-shadow: 0 10px 15px -3px rgba(var(--primary-color-rgb), 0.1);"
                            @mouseenter="$el.style.opacity = '0.85'" @mouseleave="$el.style.opacity = '1'">
